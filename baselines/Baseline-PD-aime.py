            ...
            while try_time < 3:
                # try:
                    ...
                    for T in range(num_foresight):
                        ...
                        # print(inputs_list[0])
                        sampling_params = SamplingParams(max_tokens=64, n=num_rollout, logprobs=0, temperature=0.6)
                        # 根据当前的状态，foresight
                        ...
                        # 把每一步进行补全
                        sampling_params = SamplingParams(max_tokens=128 ,n=1, logprobs=1)
                        for each_input in inputs_list: 
                            input_token_num_for_this_question += len(tokenizer(each_input)['input_ids'])
                        outputs = model.generate(inputs_list, sampling_params)
                    ...
                    sampling_params = SamplingParams(max_tokens=3000 ,n=1, logprobs=0)
                    for each_input in inputs_list: 
                        input_token_num_for_this_question += len(tokenizer(each_input)['input_ids'])
                    outputs = model.generate(inputs_list, sampling_params)
