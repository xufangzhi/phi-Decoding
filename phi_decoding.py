# cluster成两类，然后取数量较多的那一类，之后进行adv no replace sampling, 这个版本的改动是加入了early stop，使用tfidf
...
if __name__ == '__main__':
    ...
    parser.add_argument('--final_select_strategy',
                        type=str, default='same_as_strategy')
    # none, sirno_low_sigma, advno_low_sigma, advno_above_avg, adv
    parser.add_argument('--pruning_strategy', type=str,
                        default='sirno_low_sigma')
    parser.add_argument('--sigma_rate', type=float, default=1.0)
    # early stopping [around_mean, within_sigma]
    ...
    model = LLM(model=PATH_TO_CONVERTED_WEIGHTS,
                tensor_parallel_size=args.gpus, trust_remote_code=True)
        ...
        for i in range(len(test_data)):
            ...
            while try_time < 3:
                ...
                for T in range(num_foresight):
                    ...
                    cur_foresight_info = {'foresight_epoch': T, 'steps': [
                    ], 'foresight_steps': [], 'cluster_info': {}}  # 用于记录traj信息
                    ...
                    inputs_list = [inputs + reasoning_steps_list[beam_idx]
                                   for beam_idx in range(step_beam_size)]  # 对每个step beam size进行
                    # 记录一下输入的tokens数量
                    ...
                    if args.pruning_strategy == "none" or args.pruning_strategy == "":
                        ...
                        for beam_idx in range(step_beam_size):
                            for j in range(num_rollout):
                                ...
                                normalized_logp_list.append(
                                    output.cumulative_logprob / (len(output.token_ids)+1e-8))
                                adv_list.append(
                                    output.cumulative_logprob / (len(output.token_ids)+1e-8) - previous_q_value_list[beam_idx])
                        ...
                        if args.pruning_strategy == "sirno_low_sigma":
                            ...
                            if num_to_foresight < step_beam_size:
                                ...
                                weights = softmax(
                                    [logp/temp for logp in not_foresight_adv_list])
                                added_from_not_to_foresight_list = np.random.choice(
                                    len(weights), p=weights, size=num_to_add, replace=False).tolist()
                                add_idx = [not_foresight_list[i]
                                           for i in added_from_not_to_foresight_list]
                    ...
                    if args.strategy == "highest":  # select the step with highest rewards
                        selected_index = normalized_logp_list.index(
                            max(normalized_logp_list))
                        ...
                        def softmax(x):
                            ...
                            return e_x / e_x.sum(axis=0)
                        weights = softmax(
                            [logp/temp for logp in normalized_logp_list])
                        selected_index_list = np.random.choice(
                            len(weights), p=weights, size=step_beam_size).tolist()
                        ...
                        weights = softmax(
                            [logp/temp for logp in advantages_list])
                        selected_index_list = np.random.choice(
                            len(weights), p=weights, size=step_beam_size).tolist()
                        ...
                        if args.adv_above0 == "True":
                            # len(advantages_list) == len（keep_foresight_list）
                        ...
                        if len(tem_selected_index_list) < step_beam_size:
                            ...
                            try:
                                ...
                                # tem_selected_index_list中元素是在keep_foresight_list中的下标
                                tem_output_text_list = [
                                ...
                                cluster_list_foresight_idx = [
                                    [] for _ in range(k)]  # 在keep_foresight_list中的下标
                                for aidx, cluster_label in enumerate(cluster_labels):
                    ...
                    # 选出来的idx指的是在keep_foresight_list中的idx
                    sample_id_pool.append(
                        [keep_foresight_list[iiiidx] for iiiidx in selected_index_list])
                    ...
                    if args.depth_pruning_strategy != "none" and args.depth_pruning_strategy != "":
                        ...
                        if args.depth_pruning_strategy == "cluster":
                            if stop_foresight and T+1 >= args.least_foresight_num:  # 至少foresight 次数
                                print('question ', i,
                ...
                if args.final_select_strategy == "same_as_strategy":
                    if args.strategy == "cluster":
                        ...
                        if len(tem_selected_index_list) == 0:
                            ...
                            weights = softmax(
                                [-adv/temp for adv in advantages_list])  # TODO:这里加了个负号
                    ...
                    if args.strategy == "sir_no_replace":
                        ...
                        weights = softmax(
                            [logp/temp for logp in normalized_logp_list])
                        selected_index_final = np.random.choice(
                            len(weights), p=weights, replace=False)
                    elif args.strategy == "adv":
                        temp = 0.1
                    ...
                    if args.final_select_strategy == "max_logprob":
                        ...
                        avg_logprob_pool = []
                        for idx, each_step_prob_list in enumerate(each_step_prob_pool):
                            tem_avg = sum(each_step_prob_list) / \
                                len(each_step_prob_list)
                        ...
                        for idx, each_cumulative_prob_list in enumerate(all_cumulative_logprob_list):
                            ...
                            sum_avg_logprob_pool.append(
                                tem_sum/(token_num+1e-8))
            ...
            sample_id_pool.append(
                [selected_index_final for _ in range(step_beam_size)])
            ...
            whole_traj_list = [previous_steps_list[beam_idx] + "\n" + candidates_list[beam_idx]
                               for beam_idx in range(step_beam_size)]  # 每个beam的traj
