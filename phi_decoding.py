# Cluster into two categories, then take the category with more samples, followed by adv no replace sampling. This version adds early stopping using tfidf
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', type=str, default='math_500')
    ...
    parser.add_argument('--output_dir', type=str, default='./ar0.6/')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--step_beam_size', type=int, default=1)
    parser.add_argument('--num_rollout', type=int, default=1)
    parser.add_argument('--num_foresight', type=int, default=0)
    parser.add_argument('--record_process', type=bool, default=True)
    parser.add_argument('--strategy', type=str, default='cluster')
    parser.add_argument('--time_path', type=str, default='./time/') # the path to save the time information and trajectory information of each question
    ...
    parser.add_argument('--threshold', type=float, default=0.8) # threshold for early stopping
    ...
    with open(OUTPUT_PATH, "w") as f:
        ...
        for i in range(len(test_data)):
            ...
            while try_time < 3:
                # try:
                    ...
                    for T in range(num_foresight):
                        # For each question foresight times, after reaching foresight times, directly complete the remaining
                        reasoning_steps_list = previous_steps_list

                        cur_foresight_info = {'foresight_epoch':T, 'steps':[], 'foresight_steps':[], 'cluster_info':{}} # Used to record traj information
                        ...
                        inputs = tokenizer.apply_chat_template(
                            chat,
                            tokenize=False, # Output is str, not token_ids
                        )
                        # input('inputs 1')
                        # print(inputs)
                        # print(inputs)
                        ...
                        # input('inputs 1')
                        # print(inputs)
                        inputs_list = [inputs + reasoning_steps_list[beam_idx] for beam_idx in range(step_beam_size)] # For each step beam size
                        ...
                        # Foresight based on current state
                        outputs = model.generate(inputs_list, sampling_params)
                        total_rollout_times += num_rollout*step_beam_size
                        # If the prob of the rollout step is high and the variance is small, you can directly sample without foresight
                        all_avg_logp = []
                        selected_steps = []
                        inputs_list = []
                        candidates_list = []
                        ...
                        if args.pruning_strategy == "none" or args.pruning_strategy == "":
                            ...
                            # Calculate the mean of normalized_logp_list
                        ...
                        print('This foresight ', len(keep_foresight_list))
                        keep_foresight_list.sort()
                        total_rollout_times += len(keep_foresight_list) # Here is the number of foresight times added later
                        ...
                        if args.strategy == "highest":  # select the step with highest rewards
                            ...
                            if args.adv_above0 == "True":
                                ...
                                for ddi in range(len(mask)): # ddi has no specific meaning, just represents the index
                                    if mask[ddi] and output_text_list[ddi] != '': # Require adv greater than 0 and text not empty
                            ...
                            if len(tem_selected_index_list) < step_beam_size:
                                ...
                                try:
                                    ...
                                    tem_output_text_list = [output_text_list[ddi] for ddi in tem_selected_index_list] # tem_selected_index_list elements are in keep_foresight_list
                                    ...
                                    # print('selected_idx_list:', selected_index_list)
                                    # Used to record traj
                                    cluster_list_foresight_idx = [[] for _ in range(k)] # Index in keep_foresight_list
                                    for aidx, cluster_label in enumerate(cluster_labels):
                                    ...
                                    for cluster_idx, ddcluster in enumerate(cluster_list):
                                        # print('cluster_idx: ', cluster_idx, len(ddcluster))
                        ...
                        sample_id_pool.append([keep_foresight_list[iiiidx] for iiiidx in selected_index_list]) # Selected idx refers to the idx in keep_foresight_list 
                        ...
                        selected_cumulative_logprob_list = [] # Used to determine whether to continue foresight, storing data from this foresight
                    ...
                    if args.final_select_strategy == "same_as_strategy":
                        if args.strategy == "cluster":
                            ...
                            if len(tem_selected_index_list) == 0:
                                ...
                                weights = softmax([-adv/temp for adv in advantages_list]) #TODO: added a negative sign here
                                selected_index_final = np.random.choice(len(weights), p=weights)
                            else:
                                try:
                                    # The commented part is the P1+P2 method
                                    """tem_output_text_list = [output_text_list[ddi] for ddi in tem_selected_index_list] # tem_selected_index_list elements are in keep_foresight_list
                                    tem_advantages_list = [advantages_list[ddi] for ddi in tem_selected_index_list]
                                    vectorizer =TfidfVectorizer()
                                    X=vectorizer.fit_transform(tem_output_text_list)
                                    ...
                                    adv_weights = softmax([adv/temp for adv in tem_advantages_list])
                                    weights = [cluster_weights[ddi] + adv_weights[ddi] for ddi in range(len(tem_selected_index_list))]
                                    weights = [each_weight/2 for each weight in weights]
                                    selected_index_final = np.random.choice(len(weights), p=weights)
                                    selected_index_final = tem_selected_index_list[selected_index_final"""
                                    # selected_index_final = np.random.choice(len(weights), p=weights)
                                    # selected_index_final = tem_selected_index_list[selected_index_final]

                                    # The following part is to cluster into two categories, select one from the category with more samples
                                    tem_output_text_list = [output_text_list[ddi] for ddi in tem_selected_index_list] # tem_selected_index_list elements are in keep_foresight_list
                                    tem_advantages_list = [advantages_list[ddi] for ddi in tem_selected_index_list]
                                    vectorizer =TfidfVectorizer()
                                    X=vectorizer.fit_transform(tem_output_text_list)
                        ...
                        if args.final_select_strategy == "max_logprob":
                            ...
                            for idx, each_step_prob_list in enumerate(each_step_prob_pool):
                                # Find the step_beam_size maximum logprob
                                tem_max = -INF
                            ...
                            # Find the maximum from max_logprob_pool
                            max_logprob = -INF
                            ...
                            min_logprob_pool = []
                            for idx, each_step_prob_list in enumerate(each_step_prob_pool):
                                # Find the step_beam_size minimum logprob
                                tem_min = INF
                                for each_step_logprob in each_step_prob_list:
                            ...
                            # Find the minimum from min_logprob_pool
                            min_logprob = -INF
                            ...
                            # Find the maximum from max_min_diff_pool
                            max_min_diff = -INF
            ...
            whole_traj_list = [previous_steps_list[beam_idx] + "\n" + candidates_list[beam_idx] for beam_idx in range(step_beam_size)] # Traj for each beam
